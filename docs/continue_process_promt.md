Tôi muốn tiếp tục phát triển dự án cinema-web.

Trước khi thay đổi code, hãy:

1. Đọc AGENTS.md.
2. Đọc docs/PROJECT_CONTEXT.md.
3. Đọc docs/CURRENT_STATUS.md.
4. Đọc docs/ui-design-spec.md.
5. Kiểm tra git status, branch hiện tại và 10 commit gần nhất.
6. Kiểm tra package.json và cấu trúc src hiện tại.
7. Chạy các verification command đang có.
8. Báo cáo ngắn gọn:
   - trạng thái hiện tại;
   - phần đã hoàn thành;
   - vấn đề đang tồn tại;
   - công việc nhỏ nhất cần làm tiếp theo.

Không thay đổi code trước khi hoàn tất bước kiểm tra.

Sau đó tiếp tục task được ghi trong mục “Next task” của
docs/CURRENT_STATUS.md.
